...
## v2.3.0 — 2026-08-18 数据结构模板重构 & 新增树状数组笔记

- 线段树模板重构：以 Tag + Info + SegTree 解耦设计（动态开点、`std::deque` 内存池）整体替换旧模板，框架与业务信息彻底分离，换题只需重写 `Tag`/`Info`
- 树链剖分（HLD）模板同步升级：内存池 + pbds 哈希表，内置 `query_on_path` 路径操作
- 新增「树状数组 BIT」笔记：lowbit 原理（含 SVG 结构图）、代码模板、典型应用，以及与线段树的多维度辨析
- 题解代码统一收入 `Code/` 目录：P3373（线段树 2）、P3313（HLD + 线段树综合）、P3374 / P3368 / P1908（树状数组）

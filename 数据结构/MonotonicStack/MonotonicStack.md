...
### 例 4：[P2422 良好的感觉](https://www.luogu.com.cn/problem/P2422)

给定一个长度为 $n$ 的序列 $a$，对于任意子段，定义其"舒适值"为**子段和 $\times$ 子段最小值**。求最大舒适值。

**核心思路**：枚举每个元素作为最小值，用单调栈找到它左右两边第一个比它小的位置 $L[i]$ 和 $R[i]$（开区间），则该元素作为最小值时最大子段即为 $(L[i], R[i])$，配合前缀和 $O(1)$ 计算子段和，取最大值即可。
...
> 完整代码见：[Code/P2422.cpp](../../Code/P2422.cpp)
